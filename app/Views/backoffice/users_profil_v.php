...
<script>
    $(document).ready(function() {
        ...
        $('#role_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '/backoffice/profil_users/getProfil',
                method: 'POST'
            },
            order: [],
            columns: [{
    ...
    $("#add_form").submit(function(e) {
        ...
        $.ajax({
            url: '/backoffice/profil_users/insert',
            data: formData,
            dataType: 'json',
    ...
    $(document).on('click', "#edit", function(e) {
        ...
        $.ajax({
            url: '/backoffice/profil_users/edit',
            method: 'get',
            dataType: 'JSON',
            data: {
                id: id,
            ...
            success: function(response) {
                ...
                $("#id_edit").val(response.users_profil.id);
                $("#legalitas_lama").val(response.users_profil.legalitas_perusahaan);

                $("#nama_perusahaan_edit").val(response.users_profil.nama_perusahaan);
                $("#alamat_perusahaan_edit").val(response.users_profil.alamat_perusahaan);

                let users_management_data = `<option value="">--Silahkan Pilih--</option>`;

                response.users_management.forEach(function(e) {
                    users_management_data += `<option value="${e.id}"> ${e.nama_lengkap} </option>`
                });

                $("#users_id_edit").html(users_management_data);

                $("#users_id_edit").val(response.users_profil.users_id).trigger('change');
    ...
    $("#edit_form").submit(function(e) {
        ...
        let id = $('#id_edit').val();
        let legalitas_lama = $('#legalitas_lama').val();
        let users_id = $('#users_id_edit').val();
        let nama_perusahaan = $('#nama_perusahaan_edit').val();
        let alamat_perusahaan = $('#alamat_perusahaan_edit').val();
        let legalitas_perusahaan = $('#legalitas_perusahaan_edit').val();

        let formData = new FormData(this);

        formData.append('id', id);
        formData.append('legalitas_lama', legalitas_lama);
        formData.append('users_id', users_id);
        formData.append('nama_perusahaan', nama_perusahaan);
        formData.append('alamat_perusahaan', alamat_perusahaan);
        formData.append('legalitas_perusahaan', legalitas_perusahaan);
        ...
        $.ajax({
            url: '/backoffice/profil_users/update',
            data: formData,
            enctype: 'multipart/form-data',
            type: 'POST',
            contentType: false,
            ...
            success: function(response) {
                ...
                if (response.error) {
                    if (response.error.users_id) {
                        $("#users_id_edit").addClass('is-invalid');
                        $("#error-users-edit").html(response.error.users_id);
                    } else {
                        $("#users_id_edit").removeClass('is-invalid');
                        $("#error-users-edit").html('');
                    }

                    if (response.error.nama_perusahaan) {
                        $("#nama_perusahaan_edit").addClass('is-invalid');
                        $("#error-nama-perusahaan-edit").html(response.error.nama_perusahaan);
                    } else {
                        $("#nama_perusahaan_edit").removeClass('is-invalid');
                        $("#error-nama-perusahaan-edit").html('');
                    }

                    if (response.error.alamat_perusahaan) {
                        $("#alamat_perusahaan_edit").addClass('is-invalid');
                        $("#error-alamat-perusahaan-edit").html(response.error.alamat_perusahaan);
                    } else {
                        $("#alamat_perusahaan_edit").removeClass('is-invalid');
                        $("#error-alamat-perusahaan-edit").html('');
                    }

                    if (response.error.legalitas_perusahaan) {
                        $("#legalitas_perusahaan_edit").addClass('is-invalid');
                        $("#error-legalitas-edit").html(response.error.legalitas_perusahaan);
                    } else {
                        $("#legalitas_perusahaan_edit").removeClass('is-invalid');
                        $("#error-legalitas-edit").html('');
                    }
    ...
    $(document).on('click', "#delete", function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $("#id_delete").val(response.users_profil.id);
            }
        });
    });
    ...
    $("#delete_form").submit(function(e) {
        ...
        $.ajax({
            url: '/backoffice/profil_users/delete',
            method: 'post',
            dataType: 'JSON',
            data: {
                id: id,
