...
class ProfilUsersController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'users_management' => $this->usersManagementModel->getUsersManagementWhereId(session()->get('id')),
            'pengantar_perizinan' => $this->pengantarPerizinanModel->getSuratPengantar()
        ];

        return view('operator/users_profil_v', $data);
    }

    public function getProfil()
    {
        if ($this->request->isAjax()) {
            $users_profil = $this->profilUserModel->getProfilUsersWhereId(session()->get('id'));
